...
$is_course = (get_label('Subject') === 'Course');
...
// Fetch registered school sections for datalist and dropdowns
$stmtSec = $pdo->prepare("SELECT id, section_name, section_code FROM school_sections WHERE school_id = ? ORDER BY section_name ASC");
$stmtSec->execute([$active_school['id']]);
$registered_sections = $stmtSec->fetchAll();
$section_datalist = array_column($registered_sections, 'section_name');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .section-title { font-size:.75rem; text-transform:uppercase; letter-spacing:1.5px; color:#64748b; font-weight:800; margin-bottom:16px; border-left: 3px solid #2D6CDF; padding-left: 10px; }
        .existing-item { display:flex; align-items:center; gap:12px; padding:16px; background:#fff; border:1px solid #eef2fb; border-radius:14px; margin-bottom:10px; transition: .2s; }
        .existing-item:hover { background:#f8fafc; border-color: #2D6CDF; }
...
<?php endif; ?>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <div>
                ...
                <p class="text-muted small mb-0">Manage <?php echo strtolower(get_label('Classes')); ?> and <?php echo strtolower(get_label('Subjects')); ?> for <?php echo htmlspecialchars($active_school['school_name']); ?></p>
            </div>
        </div>
        ...
        <ul class="nav nav-pills mb-4 gap-2" id="acadTabs" role="tablist">
            ...
            <li class="nav-item"><button class="nav-link px-4 fw-600" data-bs-toggle="pill" data-bs-target="#tabSubjects"><i class="fas fa-book-open me-2"></i><?php echo get_label('Subjects'); ?></button></li>
                    ...
                    <div class="col-lg-7">
                        <div class="glass-card p-4">
                            ...
                            <datalist id="sectionList">
                                <?php foreach ($section_datalist as $rs): ?>
                                    <option value="<?php echo htmlspecialchars($rs); ?>">
                                <?php endforeach; ?>
                            </datalist>
                        ...
                        <div class="glass-card p-4">
                            ...
                            <div class="existing-item" id="sub-item-<?php echo $s['id']; ?>">
                                <div class="item-info">
                                    ...
                                    <div class="item-meta">
                                        Code: <strong><?php echo htmlspecialchars($s['code']); ?></strong>
                                        <?php if ($is_course): ?>
                                            &bull; Credits: <strong><?php echo $s['credit_units']; ?></strong>
                                            <?php 
                                            if ($s['semester_id']) {
                                                $semName = 'Unknown';
                                                foreach ($active_terms as $at) if ($at['id'] == $s['semester_id']) $semName = $at['name'];
                                                echo ' &bull; ' . get_label($semName);
                                            }
                                            if ($s['department_id']) {
                                                $deptName = 'Unknown';
                                            ...
                                            ?>
                                        <?php endif; ?>
                                        <?php echo $s['period'] ? ' &bull; Scheduled: '.date("M j, Y - g:i A", strtotime($s['period'])) : ''; ?>
                                    </div>
                                </div>
                                <button class="btn btn-sm btn-outline-primary" onclick="editSubject(<?php echo $s['id']; ?>,'<?php echo addslashes($s['name']); ?>','<?php echo addslashes($s['code']); ?>','<?php echo addslashes($s['period'] ?? ''); ?>','<?php echo $s['credit_units']; ?>','<?php echo $s['semester_id']; ?>','<?php echo $s['department_id']; ?>')"><i class="fas fa-pen"></i></button>
                            ...
                            <?php else: foreach ($all_sessions as $sess): ?>
                                <div class="existing-item <?php echo $active_school['current_session_id'] == $sess['id'] ? 'border-primary bg-light' : ''; ?>">
                                    <div class="item-info">
                                        ...
                                        <div class="item-meta"><?php echo $active_school['current_session_id'] == $sess['id'] ? 'Currently Active' : 'Status: ' . ucfirst($sess['status']); ?></div>
                                    </div>
                                    <div class="d-flex gap-1">
                                        <?php if ($sess['status'] === 'active' && $active_school['current_session_id'] != $sess['id']): ?>
                                            <button class="btn btn-sm btn-outline-success border-0" onclick="setActiveSession(<?php echo $sess['id']; ?>)" title="Make Active"><i class="fas fa-play"></i></button>
                                        <?php endif; ?>
...
function addClassRow(id='', name='', code='', section='', seq='0') {
    ...
    const html = `<div class="builder-row" id="crow-${rid}">
        <div class="row g-3 align-items-center">
            ...
            ${IS_COURSE ? '' : codeField}
            ${IS_COURSE ? '' : sectionField}
            <div class="col-md-2" title="Sequence for Promotion (1, 2, 3...)">
                <label class="extra-small fw-800 text-muted text-uppercase mb-1 d-block">Sequence</label>
                <div class="input-group">
                    <span class="input-group-text bg-white border-end-0"><i class="fas fa-sort-numeric-down text-muted"></i></span>
        ...
        ${IS_COURSE ? codeField + sectionField : ''}
    </div>`;
...
// ---- SUBJECT BUILDER ----
function addSubjectRow(id='', name='', code='', period='', credits='2', sem_id='', dept_id='') {
    const rid = ++subRowId;
    ...
    if (IS_COURSE) {
        tertiaryFields = `
            <div class="col-md-2">
                <label class="extra-small fw-800 text-muted text-uppercase mb-1 d-block">Credits</label>
    ...
    const html = `<div class="builder-row" id="srow-${rid}">
        <div class="row g-3 align-items-center">
            ...
            <div class="col-md-${IS_COURSE ? '4' : '4'}">
                <label class="extra-small fw-800 text-muted text-uppercase mb-1 d-block">${SUBJECT_NAME_LABEL} *</label>
                <input class="form-control" placeholder="${SUBJECT_NAME_LABEL} *" value="${name}" data-field="name" title="Name">
            </div>
            <div class="col-md-${IS_COURSE ? '2' : '3'}">
                <label class="extra-small fw-800 text-muted text-uppercase mb-1 d-block">Code *</label>
                <input class="form-control" placeholder="${SUBJECT_CODE_LABEL} *" value="${code}" data-field="code" title="Code">
            </div>
            ${tertiaryFields}
            <div class="col-md-${IS_COURSE ? '4' : '3'}">
                <label class="extra-small fw-800 text-muted text-uppercase mb-1 d-block">${periodLabel} *</label>
                <input class="form-control time-picker" placeholder="${periodLabel} *" value="${period}" data-field="period" readonly title="Schedule">
            </div>
            <div class="col-md-2 text-end pt-4">
                <button class="btn btn-outline-danger border-0 rounded-circle" onclick="document.getElementById('srow-${rid}').remove()">
                    <i class="fas fa-circle-minus fa-xl"></i>
                </button>
            </div>
        </div>
    ...
    if (sem_id) {
        document.querySelector(`#srow-${rid} [data-field="semester"]`).value = sem_id;
    }
    ...
    if (dept_id) {
        document.querySelector(`#srow-${rid} [data-field="department"]`).value = dept_id;
    }
}
...
function saveSubjects() {
    ...
    const subjects = [];
    rows.forEach(r => {
        const creditEl = r.querySelector('[data-field="credits"]');
        const semEl = r.querySelector('[data-field="semester"]');
        subjects.push({
            ...
            credit_units: creditEl ? creditEl.value : 0,
            semester_id: semEl ? semEl.value : 0,
            department_id: r.querySelector('[data-field="department"]') ? r.querySelector('[data-field="department"]').value : 0
        });
    });
...
function editSubject(id, name, code, period, credits, sem_id, dept_id) {
    document.getElementById('subjectBuilderRows').innerHTML = '';
    addSubjectRow(id, name, code, period, credits, sem_id, dept_id);
