<?php
// user/support_view.php - Technical Thread Orchestrator (Staff)
require_once '../includes/auth_check.php';

$ticket_id = $_GET['id'] ?? null;
if (!$ticket_id) { header('Location: support.php'); exit(); }

// Access verification - ensures staff can only see their school's tickets or tickets they created
$stmt = $pdo->prepare("SELECT r.*, s.school_name FROM school_requests r JOIN schools s ON r.school_id = s.id WHERE r.id = ? AND (r.school_id = ? OR r.user_id = ?)");
$stmt->execute([$ticket_id, $_SESSION['school_id'] ?? 0, $user_id]);
$ticket = $stmt->fetch();
...
<head>
    ...
    <title>#TKT-<?php echo $ticket_id; ?> | Staff Support Hub</title>
    ...
    <style>
        ...
        .bubble-staff { background: #f1f5f9; margin-right: auto; border-bottom-left-radius: 4px; border: 1px solid #e2e8f0; }
                ...
                <div class="col-lg-12">
                    <div class="glass-card p-0 overflow-hidden border-0 shadow-sm">
                        ...
                        <div class="p-4 bg-light bg-opacity-50" style="min-height: 400px; max-height: 600px; overflow-y: auto;" id="chatThread">
                            <!-- Initial Message -->
                            <div class="chat-bubble bubble-staff reveal-fade-up">
                            ...
                            <?php foreach($messages as $msg): 
                                ...
                                $bubble_class = $is_system ? 'bubble-system bg-white border-dashed text-center w-100 mx-0 opacity-75' : (($msg['sender_role'] == 'super_admin') ? 'bubble-sa' : 'bubble-staff');
                                $sender_label = ($msg['sender_role'] == 'super_admin') ? 'Orchestration Expert' : 'Institutional Staff';
                            ?>
                            <?php if($is_system): ?>
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <?php else: ?>
                                            <a href="../<?php echo $msg['file_path']; ?>" target="_blank" class="btn btn-white border shadow-sm rounded-4 p-3 d-flex align-items-center gap-3 text-decoration-none text-dark hover-translate-y">
                                                ...
                                                <div>
                                                    <div class="fw-800 tiny-text">Open Attachment</div>
                                                </div>
                                            </a>
                                        <?php endif; ?>
                                    </div>
